...
//    private let queue = DispatchQueue(label: "SynchronizedArray_\(UUID().uuidString)", attributes: .concurrent)
...
//
//            queue.sync {
//                result = self.array.indices.contains(index) ? self[index] : nil
//            }
//            return result
//        }
//
...
//        queue.async(flags: .barrier) {
//            guard let index = self.array.firstIndex(of: element) else { return }
